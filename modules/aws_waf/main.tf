...
resource "aws_wafv2_web_acl" "waf" {
  ...
  rule {
    name     = "sas-awsng-GeoRestrictionRule"
    priority = 2
    action {
      block {}
    }
  ...
  rule {
    name     = "sas-awsng-IpReputationList"
    priority = 4
    statement {
      managed_rule_group_statement {
  ...
  # Known Bad Inputs
  rule {
    name     = "sas-awsng-Bad-Inputs-Rule"
    priority = 5
    statement {
      managed_rule_group_statement {
  ...
  tags = merge(
    {
      "Name" = format("%s", "sas-awsng-${var.spoke_account_id}-acl")
    },
    var.tags
  )
}
